 ...
 * 원가/평균단가는 이 계산 결과를 쓰므로, 둘의 규칙이 흔들리면 화면 숫자가 서로 어긋난다(실측 사례: 매매 화면 헤드라인과 계좌별 표가 0.11% 차이).
 *
 ...
 * 평단 11,767.3 으로 네 건이 모두 1 원 오차 없이 맞는다.
 *
 * <p>그래서 계좌별 표의 매도원가(= 매도금액 - 거래세 - 기록 실현손익)는 그 계좌의 매수와 맞지 않을 수 있다. 연금저축1 의 PLUS 고배당주는 403 주를
 * 14,805 원에 사서 19,895 원에 팔았는데, 기록 실현손익에서 역산하면 매도원가가 주당 18,888 원이다(평단보다 27.6% 높다). 같은 화면에 이 계산기가 낸
 * 평단 14,805 원과 나란히 나온다. 이 계산기를 고칠 때 그 차이를 "버그" 로 오해하지 않도록 적어 둔다.
 */
