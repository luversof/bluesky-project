...
public interface DailyAccountSnapshotRepository extends CrudRepository<DailyAccountSnapshot, UUID> {
  ...
  @Modifying
  @Query(
      "DELETE FROM \"DailyAccountSnapshot\" WHERE \"date\" >= :fromDate AND \"wmaState\" IS NOT NULL AND \"wmaState\"::text LIKE CONCAT('%', :stockItemId, '%')")
  ...
  @Query(
      "SELECT * FROM \"DailyAccountSnapshot\" WHERE \"user_id\" = :userId AND \"account_id\" = :accountId AND \"date\" <= :date ORDER BY \"date\" DESC LIMIT 1")
  java.util.Optional<DailyAccountSnapshot> findLatestByUserIdAndAccountIdOnOrBefore(
      @Param("userId") UUID userId,
      @Param("accountId") UUID accountId,
