 ...
 * 전체 값이었다. 요약 화면이 그것을 '누적 확정 수익'으로 쓰는 바람에 필터를 건 실현손익과 어긋났다(실측: KB증권 위탁 자리에 그 계좌 값의 6.1 배인 전 계좌 합계가
 * 들어갔다). 필터가 반영된 합계는 {@code GET /api/dividend/total} 이 손익 조회와 같은 파라미터로 낸다.
 */
