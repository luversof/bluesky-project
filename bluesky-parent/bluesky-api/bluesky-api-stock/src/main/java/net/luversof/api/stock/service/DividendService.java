...
public class DividendService {
   ...
   * 같은 조건의 세후 배당 합계.
   *
   * <p>게이트 요약 화면이 계좌/종목 필터를 건 상태에서도 배당만 전체 합계를 쓰고 있었다(실측: 계좌 하나를 골라도 '누적 확정 수익' 에 전체 배당 61,645,687
   * 이 그대로 더해져 KB증권 위탁이 10,113,820 대신 61,646,257 로 표시됐다). 실현손익과 같은 조건으로 집계해 두 항이 어긋나지 않게 한다.
   */
   ...
   * 실제로 이익인데 손실로 세어진다(실측 2026-08-24: TIGER 리츠부동산인프라 실현+평가 -2,929,544 인데 배당 5,132,889 이라 실제로는
   * +2,203,345 - 42 종목 중 1 종목이 뒤집혀 76.19% 대신 78.57%).
   *
