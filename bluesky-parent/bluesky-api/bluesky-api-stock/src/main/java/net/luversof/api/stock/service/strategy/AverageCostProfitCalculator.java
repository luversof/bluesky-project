...
public class AverageCostProfitCalculator implements ProfitCalculator {
	...
	@Override
	public TradeProfit calculate(List<Trade> trades, TradeProfitRequest request, StockPriceService stockPriceService) {
		...
		UUID accountId = (request.getGroupBy() == TradeProfitRequestGroup.ACCOUNT_AND_STOCKITEM) ? first.getAccountId()
				: null;
		...
		for (Trade trade : sortedTrades) {
			...
			if (start != null && tradeDate.isBefore(start))
				inPeriod = false;
			if (end != null && tradeDate.isAfter(end))
				inPeriod = false;
			...
			if (trade.getType() == TradeType.BUY) {
				...
				if (currentQuantity > 0) {
					...
					unitCostGross = currentTotalCost.divide(BigDecimal.valueOf(currentQuantity), 10,
							RoundingMode.HALF_UP);
				}
		...
		BigDecimal averageBuyPriceNet = holdingQuantity > 0
				? currentTotalCostNet.divide(BigDecimal.valueOf(holdingQuantity), 2, RoundingMode.HALF_UP)
				: BigDecimal.ZERO;
		...
		log.debug("[ProfitCalculator] Stock: {}, Realized: {}, RealizedNet: {}", stockItemId, periodRealizedProfit,
