...
public class StockPriceService {
	...
	public BigDecimal getCurrentPrice(UUID stockItemId) {
		return stockPriceRepository.findByStockItemId(stockItemId).map(StockPrice::getPrice).orElse(BigDecimal.ZERO);
	}
	...
	public BigDecimal getPriceAt(UUID stockItemId, LocalDate at) {
		...
		return stockPriceHistoryRepository
				...
				.map(StockPriceHistory::getClosePrice).orElseGet(() -> getCurrentPrice(stockItemId));
	}

	public StockPrice savePrice(UUID stockItemId, BigDecimal price) {
		StockPrice stockPrice = stockPriceRepository.findByStockItemId(stockItemId).orElse(new StockPrice());
	...
	public List<StockPriceHistory> getPriceHistory(Collection<UUID> stockItemIdList, LocalDate start, LocalDate end) {
		return stockPriceHistoryRepository.findByStockItemIdInAndTradeDateBetween(stockItemIdList, start, end);
	}
}
