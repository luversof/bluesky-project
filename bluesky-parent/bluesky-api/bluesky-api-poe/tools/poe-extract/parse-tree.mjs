...
// 트리는 항상 GGG master 에서 최신본을 받는다 — RAW 캐시가 있으면 재fetch 를 건너뛰던 옛 로직은
// 게임 데이터가 갱신돼도 트리를 옛 스냅샷에 고정시켰다(실측: 7/14 fetch 3337노드 vs GGG master 3389노드).
...
try {
	...
	console.warn("트리 다운로드 실패 — 기존 캐시로 폴백:", e.message);
}
