  ...
  void createEntry() {
    ...
    var contraAsset =
        assetList.stream()
            .filter(
                asset ->
                    getAssetType(asset.getAssetTypeId()).getCode() == AssetTypeCode.CONTRA_ASSET)
            .findAny()
            .get();
    var cashAsset =
        assetList.stream()
            .filter(asset -> getAssetType(asset.getAssetTypeId()).getCode() == AssetTypeCode.CASH)
            .findAny()
            .get();
    var incomeEntryType =
        entryTypeList.stream()
            .filter(entryType -> entryType.getCode() == EntryTypeCode.INCOME)
            .findAny()
            .get();
    var outgoingEntryType =
        entryTypeList.stream()
            .filter(entryType -> entryType.getCode() == EntryTypeCode.OUTGOING)
            .findAny()
            .get();
