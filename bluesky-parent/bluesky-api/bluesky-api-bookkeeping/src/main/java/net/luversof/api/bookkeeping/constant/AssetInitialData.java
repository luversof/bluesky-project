...
public enum AssetInitialData {
    CONTRA_ASSET(
            AssetTypeInitialData.CONTRA_ASSET.getCode(),
            AssetJsonConfigConstant.getImmutableConfigList()),
    ...
    public String getLocalizedName() {
        return MessageUtil.getMessage(
                MessageFormat.format("bookkeeping.constant.account.{0}", name()), name());
    }

    public static List<Asset> getInitialData(
            Bookkeeping bookkeeping, List<AssetType> assetTypeList) {
        var assetList = new ArrayList<Asset>();

        for (var assetInitialData : AssetInitialData.values()) {
            var targetAssetType =
                    assetTypeList.stream()
                            .filter(
                                    accountType ->
                                            assetInitialData.assetTypeCode.equals(
                                                    accountType.getCode()))
                            .findFirst()
                            .orElseGet(() -> null);
