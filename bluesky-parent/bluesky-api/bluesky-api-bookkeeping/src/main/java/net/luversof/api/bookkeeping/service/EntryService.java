...
public class EntryService {
	...
	public Entry updateEntry(Entry entry) {
		...
		var targetEntry = entryRepository.findById(entry.getId())
	...
	private void checkEntry(Entry entry) {
		...
		var bookkeeping = bookkeepingService.findById(entry.getBookkeepingId())
				.orElseThrow(BookkeepingErrorCode.NOT_EXIST_BOOKKEEPING::exception);

		// 2. incomeAsset 확인
		var incomeAsset = assetService.findById(entry.getIncomeAssetId())
		...
		// 3. outgoingAsset 확인
		var outgoingAsset = assetService.findById(entry.getOutgoingAssetId())
				.orElseThrow(BookkeepingErrorCode.NOT_EXIST_ASSET::exception);
		...
		boolean isTransfer = outgoingAssetType.getCode() != AssetTypeCode.CONTRA_ASSET
		...
		if (!isTransfer) {
			var entryType = entryTypeRepository.findById(entry.getEntryTypeId())
			...
			if ((outgoingAssetType.getCode() == AssetTypeCode.CONTRA_ASSET
					&& entryType.getCode() != EntryTypeCode.INCOME) // 수입인 경우 entryType이 수입 관련 코드인지 확인
					|| (incomeAssetType.getCode() == AssetTypeCode.CONTRA_ASSET
							&& entryType.getCode() != EntryTypeCode.OUTGOING) // 지출인 경우 entryType이 지출 관련 코드인지 확인
			) {
