...
$(document).ready(function() {
	 ...
	 */
	var view = new View({
		template : $("#entryGroup-template").text(),
	...
	var controller = new Controller({
		url : "/user/1/bookkeeping/entryGroup",
		view : view,
		initialize : function() {
			$(".entryGroup-menu-add").css("cursor", "pointer");
		},
		events : {
			"tr" : {
		...
		externalEvents : {
			".entryGroup-menu-add" : { "click" : "meunAddDisplay" },
			".entryGroup-add-modal .entryGroup-add" : { "click" : "add" }
		},
		/** (s) event **/
		...
		getDataFromTemplate : function(target) {
			...
			return {
				...
				entryType : target.find("td:eq(1)").text()
			};
		},
		...
		add : function(event) {
			...
			var model = new Model(entryGroup, {controller : event.data.controller});
...
//	var data2 = new Model({asset : {id : 1}, entryGroupGroup : { id : 1}, amount : 123, memo : "test"}, {controller : controller});
//	data2.add();
});