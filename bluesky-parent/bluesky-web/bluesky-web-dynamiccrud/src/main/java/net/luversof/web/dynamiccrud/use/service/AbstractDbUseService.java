...
public abstract class AbstractDbUseService implements UseService {
  ...
  /**
   * 사용자가 입력한 임의 SQL을 실행한다(벌크 쿼리 실행 기능). DataSource는 해당 subMenu에 등록된 SELECT 쿼리의 연결명을 그대로 사용한다.
   * JSqlParser로 파싱해 SELECT면 결과 리스트, 그 외(DML)면 영향 행수를 반환한다. 파싱 실패(잘못된 SQL)는 예외로 던져 호출부에서 사용자에게 표시한다.
              ...
              dynamicCrudSettingTransactionHandler.runInReadUncommittedTransaction(
                  () ->
                      namedParameterJdbcTemplate.query(
                          single, new MapSqlParameterSource(), ROW_MAPPER));
