...
public final class SettingStringUtil {

    private SettingStringUtil() {}
    ...
    // Pattern p =
    // Pattern.compile("FROM\\s+(?:\\w+\\.)*(\\w+)(\\s*$|\\s+(WITH|WHERE|[LEFT|RIGHT]+\\s+[OUTER|INNER]+\\s+JOIN|ORDER\\s+BY|GROUP\\s+BY|HAVING|LIMIT|OFFSET))",Pattern.CASE_INSENSITIVE);
    ...
    // Pattern p =
    // Pattern.compile("(ORDER\\s+BY.*?)(\\s*$|\\s+([LEFT|RIGHT]+\\s+[OUTER|INNER]+\\s+JOIN|ORDER\\s+BY|GROUP\\s+BY|HAVING|LIMIT))",Pattern.CASE_INSENSITIVE);
    // Matcher result = p.matcher(query);
    // if (result.find()) {
    // return result.group(1);
    // }
