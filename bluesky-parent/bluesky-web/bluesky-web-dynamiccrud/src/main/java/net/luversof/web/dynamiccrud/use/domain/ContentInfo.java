...
public class ContentInfo {
  ...
  /**
   * view 페이지 input form 처리를 위해 원본 데이터를 같이 전달해야 함. 이 때 db data에는 column type SPEL인 값이 존재하지 않으므로 해당
   * 값도 같이 추가해서 전달 근데 이게 의미가 있나? db에 저장하는 data가 아니고 field 정의에 저장되는 값인데... SPEL의 경우 update가 불가하도록 처리가
   * 필요할 듯 함
   */
  ...
  public List<ContentKeyInfo> getContentKeyInfoList() {
    return contentKeyInfoList;
  }

  public List<Map<String, Object>> getOriginContentMapList() {
    return originContentMapList;
  }

  public List<Map<String, Object>> getProcessedContentMapList() {
    return processedContentMapList;
  }
   ...
   */
  public ContentInfo(
      SubMenuDbType dbType, List<Map<String, Object>> contentMapList, List<DbField> dbFieldList) {
  ...
  private void createContentKeyInfoList(List<DbField> dbFieldList) {
        ...
        .forEach(
            key -> {
              ...
              if (dbFieldList == null || dbFieldList.isEmpty()) {
                contentKeyInfoList.add(
                    new ContentKeyInfo(
                        key, key, Short.MAX_VALUE, DbFieldColumnType.STRING, true, null));
                return;
              }
              ...
              var targetField =
                  dbFieldList.stream()
                      .filter(x -> x.getColumnId().equals(key))
                      .findAny()
                      .orElseGet(() -> null);
              if (targetField == null) {
                contentKeyInfoList.add(
              ...
              if (!targetField.getColumnVisible().equals(DbFieldVisible.NOT_USED)) {
                contentKeyInfoList.add(
                    new ContentKeyInfo(
                        targetField.getColumnId(),
                        targetField.getColumnName(),
                        targetField.getColumnOrder(),
                        targetField.getColumnType(),
                        targetField.isColumnVisible(),
                        targetField));
              }
            });
    ...
    dbFieldList.stream()
        .filter(x -> x.getColumnType().equals(DbFieldColumnType.SPEL))
        .forEach(
            dbField -> {
              contentKeyInfoList.add(
                  new ContentKeyInfo(
                      ...
                      dbField.getColumnOrder(),
                      dbField.getColumnType(),
                      dbField.isColumnVisible(),
                      dbField));
            });
    ...
    for (var originContentMap : this.originContentMapList) {
      ...
      for (var contentKeyInfo : contentKeyInfoList) {
        ...
        if (contentKeyInfo.type().equals(DbFieldColumnType.SPEL)) {
          ...
          processedContentMap.put(
              contentKeyInfo.key(),
              getProcessedValue(originContentMap, contentKeyInfo.originKey(), dbField));
        }
      }
   ...
   * @return
   */
  private Object getProcessedValue(
      Map<String, Object> originContentMap, String originKey, DbField dbField) {
    ...
    if (dbType == SubMenuDbType.Mongo
        && dbField.getColumnType() == DbFieldColumnType.DATE
        && originValue instanceof Date date) {
