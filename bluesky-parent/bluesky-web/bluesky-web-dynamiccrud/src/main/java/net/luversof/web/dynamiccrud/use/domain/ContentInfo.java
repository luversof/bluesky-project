...
public class ContentInfo {
	...
	private void createContentKeyInfoList(List<DbField> dbFieldList) {
		...
		firstContent.keySet().forEach(key -> {
			...
			if (dbFieldList == null || dbFieldList.isEmpty()) {
				contentKeyInfoList
						.add(new ContentKeyInfo(key, key, Short.MAX_VALUE, DbFieldColumnType.STRING, true, null));
				return;
			}

			// 저장된 dbField 목록에 해당 key에 대한 정보가 없으면 데이터의 key를 그대로 사용
			var targetField = dbFieldList.stream().filter(x -> x.getColumnId().equals(key)).findAny()
					.orElseGet(() -> null);
			if (targetField == null) {
				contentKeyInfoList
						.add(new ContentKeyInfo(key, key, Short.MAX_VALUE, DbFieldColumnType.STRING, true, null));
			...
			if (!targetField.getColumnVisible().equals(DbFieldVisible.NOT_USED)) {
				contentKeyInfoList.add(new ContentKeyInfo(targetField.getColumnId(), targetField.getColumnName(),
						targetField.getColumnOrder(), targetField.getColumnType(), targetField.isColumnVisible(),
						targetField));
			}
		});
		...
		dbFieldList.stream().filter(x -> x.getColumnType().equals(DbFieldColumnType.SPEL)).forEach(dbField -> {
			contentKeyInfoList.add(new ContentKeyInfo(dbField.getColumnId(), dbField.getColumnName(),
					dbField.getColumnOrder(), dbField.getColumnType(), dbField.isColumnVisible(), dbField));
		});
		...
		for (var originContentMap : this.originContentMapList) {
			...
			for (var contentKeyInfo : contentKeyInfoList) {
				...
				if (contentKeyInfo.type().equals(DbFieldColumnType.SPEL)) {
					...
					processedContentMap.put(contentKeyInfo.key(),
	...
	private Object getProcessedValue(Map<String, Object> originContentMap, String originKey, DbField dbField) {
		 ...
		 */
		if (dbType == SubMenuDbType.Mongo && dbField.getColumnType() == DbFieldColumnType.DATE
