...
public interface SettingServiceDecorator {
     ...
     * @return
     */
    default <T extends Setting, S extends SettingService<T>>
            Map<String, S> getSortedSettingServiceMap(Map<String, S> settingServiceMap) {
        return settingServiceMap.entrySet().stream()
                .sorted(Comparator.comparing(e -> e.getValue().getOrder()))
                .collect(
                        Collectors.toMap(
                                Entry::getKey, Entry::getValue, (_, y) -> y, LinkedHashMap::new));
    }
}
