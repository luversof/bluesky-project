...
public class UseExcelView extends AbstractXlsxView {
    ...
    protected void buildExcelDocument(
        ...
        // col data 설정
        {
            List<Map<String, Object>> processedContentMapList =
                    contentInfo.getProcessedContentMapList();
        ...
        // width 설정 (데이터가 모두 추가된 이후 설정해야 적용됨)
        {
            ...
            var contentKeyInfo : contentInfo.getContentKeyInfoList()) {
    ...
    private CellStyle createCellStyle(Workbook workbook, CellType cellType) {
        ...
        if (cellType == CellType.HEADER) {
            cellStyle.setFillForegroundColor(
                    new XSSFColor(new Color(231, 234, 246), new DefaultIndexedColorMap()));
            cellStyle.setAlignment(HorizontalAlignment.CENTER);
            cellStyle.setVerticalAlignment(VerticalAlignment.CENTER);
            font.setBold(true);
        } else if (cellType == CellType.CONTENT) {
            cellStyle.setFillForegroundColor(
                    new XSSFColor(new Color(255, 255, 255), new DefaultIndexedColorMap()));
        }
        return cellStyle;
    }
