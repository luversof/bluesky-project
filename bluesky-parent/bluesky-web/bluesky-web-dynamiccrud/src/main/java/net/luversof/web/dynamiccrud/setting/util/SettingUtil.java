...
public final class SettingUtil extends RequestAttributeUtil {
  ...
  /**
   * setting Parameter의 조회 1. attribute에 캐싱된 값 조회 2. HandlerMapping의
   * URI_TEMPLATE_VARIABLES_ATTRIBUTE 값 조회 3. filter의 경우 위 2번이 설정되기 전 단계이기 때문에 별도의
   * ServletRequestUtil로 조회
   *
   * @return
   */
  ...
  public static SettingParameter getSettingParameter() {
    ...
    var uriTemplateVariablesAttribute =
        getRequestAttribute(HandlerMapping.URI_TEMPLATE_VARIABLES_ATTRIBUTE);
    ...
    settingParameterOptional =
        Optional.of(
            new SettingParameter(
                isSettingUrl
                    ? AdminConstant.ADMIN_PROJECT_ID_VALUE
  ...
  @SuppressWarnings("unchecked")
  public static <T extends Setting> SettingServiceSupplier<T> getSettingServiceSupplierDecorator(
      Class<T> clazz) {
    ResolvableType type = ResolvableType.forClassWithGenerics(SettingServiceSupplier.class, clazz);
    return (SettingServiceSupplier<T>)
        ApplicationContextUtil.getApplicationContext().getBeanProvider(type).stream()
            .filter(x -> x instanceof SettingServiceDecorator)
            .findFirst()
            .orElseGet(() -> null);
  }

  @SuppressWarnings("unchecked")
  public static <T extends Setting>
      SettingServiceListSupplier<T> getSettingServiceListSupplierDecorator(Class<T> clazz) {
    ResolvableType type =
        ResolvableType.forClassWithGenerics(SettingServiceListSupplier.class, clazz);
    return (SettingServiceListSupplier<T>)
        ApplicationContextUtil.getApplicationContext().getBeanProvider(type).stream()
            .filter(x -> x instanceof SettingServiceDecorator)
            .findFirst()
            .orElseGet(() -> null);
  ...
  public static Project getProject(SettingParameter settingParameter) {
    if (settingParameter == null
        || settingParameter.adminProjectId() == null
        || settingParameter.projectId() == null) {
      return null;
    }

    var attributeName =
        getAttributeName(PROJECT, settingParameter.adminProjectId(), settingParameter.projectId());
    return getObject(
        attributeName,
  ...
  public static MainMenu getMainMenu(SettingParameter settingParameter) {
    if (settingParameter == null
        || settingParameter.adminProjectId() == null
        || settingParameter.projectId() == null
        || settingParameter.mainMenuId() == null) {
      return null;
    }
    ...
    var attributeName =
        getAttributeName(
            MAINMENU,
            settingParameter.adminProjectId(),
  ...
  public static List<MainMenu> getMainMenuList(SettingParameter settingParameter) {
    ...
    var attributeName =
        getAttributeName(
            MAINMENU_LIST, settingParameter.adminProjectId(), settingParameter.projectId());
    return getList(
        attributeName,
  ...
  public static SubMenu getSubMenu(SettingParameter settingParameter) {
    return getSubMenuList(settingParameter).stream()
        .filter(x -> x.getSubMenuId().equals(settingParameter.subMenuId()))
        .findAny()
  ...
  public static SubMenu getSubMenu(
    ...
    return getSubMenuList(adminProjectId, projectId, mainMenuId).stream()
        .filter(x -> x.getSubMenuId().equals(subMenuId))
        .findAny()
        .orElseThrow(() -> new BlueskyException("NOT_EXIST_SELECT_SUBMENU"));
  }
  ...
  public static DbQuery getDbQuery(
      SettingParameter settingParameter, DbQuerySqlCommandType sqlCommandType) {
    List<DbQuery> queryList = getDbQueryList(settingParameter);
    return queryList.stream()
        .filter(x -> x.getSqlCommandType().equals(sqlCommandType))
        .findAny()
  ...
  public static DbQuery getDbQuery(
    ...
    return getDbQuery(
        new SettingParameter(adminProjectId, projectId, mainMenuId, subMenuId), sqlCommandType);
  }
