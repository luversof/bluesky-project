 ...
 */
@DevCheckUtil
public final class SettingUtil extends RequestAttributeUtil {
	...
	public static SettingParameter getSettingParameter() {
		...
		settingParameterOptional = Optional.of(
				new SettingParameter(
						isSettingUrl ? AdminConstant.ADMIN_PROJECT_ID_VALUE
								: pathVariableMap.get(SettingConstant.ADMIN_PROJECT_ID),
	...
	public static <T extends Setting> SettingServiceSupplier<T> getSettingServiceSupplierDecorator(Class<T> clazz) {
		...
		return (SettingServiceSupplier<T>) ApplicationContextUtil.getApplicationContext().getBeanProvider(type).stream()
				.filter(x -> x instanceof SettingServiceDecorator).findFirst().orElseGet(() -> null);
	}

	@SuppressWarnings("unchecked")
	public static <T extends Setting> SettingServiceListSupplier<T> getSettingServiceListSupplierDecorator(
			Class<T> clazz) {
		ResolvableType type = ResolvableType.forClassWithGenerics(SettingServiceListSupplier.class, clazz);
		return (SettingServiceListSupplier<T>) ApplicationContextUtil.getApplicationContext().getBeanProvider(type)
				.stream().filter(x -> x instanceof SettingServiceDecorator).findFirst().orElseGet(() -> null);
	}
	...
	public static AdminProject getAdminProject(SettingParameter settingParameter) {
		...
		return getObject(attributeName,
				() -> getSettingServiceSupplierDecorator(AdminProject.class).findOne(settingParameter));
	}
	...
	public static Project getProject(SettingParameter settingParameter) {
		if (settingParameter == null || settingParameter.adminProjectId() == null
				|| settingParameter.projectId() == null) {
			return null;
		}

		var attributeName = getAttributeName(PROJECT, settingParameter.adminProjectId(), settingParameter.projectId());
		return getObject(attributeName,
				() -> getSettingServiceSupplierDecorator(Project.class).findOne(settingParameter));
	}
	...
	public static MainMenu getMainMenu(SettingParameter settingParameter) {
		if (settingParameter == null || settingParameter.adminProjectId() == null
				|| settingParameter.projectId() == null || settingParameter.mainMenuId() == null) {
			return null;
		}

		var attributeName = getAttributeName(MAINMENU, settingParameter.adminProjectId(), settingParameter.projectId(),
				settingParameter.mainMenuId());
		return getObject(attributeName,
				() -> getSettingServiceSupplierDecorator(MainMenu.class).findOne(settingParameter));
	}
	...
	public static List<MainMenu> getMainMenuList(SettingParameter settingParameter) {
		...
		var attributeName = getAttributeName(MAINMENU_LIST, settingParameter.adminProjectId(),
				settingParameter.projectId());
		return getList(attributeName,
				() -> getSettingServiceListSupplierDecorator(MainMenu.class).findList(settingParameter));
	}
	...
	public static List<SubMenu> getSubMenuList(SettingParameter settingParameter) {
		...
		var attributeName = getAttributeName(SUBMENU_LIST, settingParameter.adminProjectId(),
				settingParameter.projectId(), settingParameter.mainMenuId());
		return getList(attributeName,
				() -> getSettingServiceListSupplierDecorator(SubMenu.class).findList(settingParameter));
	}
	...
	public static SubMenu getSubMenu(SettingParameter settingParameter) {
		return getSubMenuList(settingParameter).stream()
				.filter(x -> x.getSubMenuId().equals(settingParameter.subMenuId())).findAny()
				.orElseThrow(() -> new BlueskyException("NOT_EXIST_SELECT_SUBMENU"));
	}

	public static SubMenu getSubMenu(String adminProjectId, String projectId, String mainMenuId, String subMenuId) {
		return getSubMenuList(adminProjectId, projectId, mainMenuId).stream()
				.filter(x -> x.getSubMenuId().equals(subMenuId)).findAny()
				.orElseThrow(() -> new BlueskyException("NOT_EXIST_SELECT_SUBMENU"));
	}
	...
	public static List<DbQuery> getDbQueryList(SettingParameter settingParameter) {
		if (settingParameter == null || settingParameter.adminProjectId() == null
				|| settingParameter.projectId() == null || settingParameter.mainMenuId() == null
				|| settingParameter.subMenuId() == null) {
			return Collections.emptyList();
		}

		var attributeName = getAttributeName(DBQUERY_LIST, settingParameter.adminProjectId(),
				settingParameter.projectId(), settingParameter.mainMenuId(), settingParameter.subMenuId());
		return getList(attributeName,
				() -> getSettingServiceListSupplierDecorator(DbQuery.class).findList(settingParameter));
	}
	...
	public static DbQuery getDbQuery(SettingParameter settingParameter, DbQuerySqlCommandType sqlCommandType) {
		...
		return queryList.stream().filter(x -> x.getSqlCommandType().equals(sqlCommandType)).findAny()
				.orElseThrow(() -> new BlueskyException("NOT_EXIST_QUERY_TYPE", sqlCommandType.toString()));
	}

	public static DbQuery getDbQuery(String adminProjectId, String projectId, String mainMenuId, String subMenuId,
	...
	public static List<DbField> getDbFieldList(SettingParameter settingParameter) {
		...
		var attributeName = getAttributeName(DBFIELD_LIST, settingParameter.adminProjectId(),
				settingParameter.projectId(), settingParameter.mainMenuId(), settingParameter.subMenuId());
		return getList(attributeName,
				() -> getSettingServiceListSupplierDecorator(DbField.class).findList(settingParameter));
	}

	public static List<DbField> getDbFieldList(String adminProjectId, String projectId, String mainMenuId,
			String subMenuId) {
