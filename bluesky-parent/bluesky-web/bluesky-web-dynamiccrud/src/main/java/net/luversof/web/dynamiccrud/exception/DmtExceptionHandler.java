...
/**
 * bluesky boot의 기본 처리는 4xx를 에러 페이지(/support/404 등)로 redirect 하기 때문에 BlueskyException에 담긴
 * 에러 메시지가 화면에 표시되지 않는다. htmx fragment 요청에 한해 메시지를 담은 fragment를 직접 반환한다.
 */
@Order(Ordered.LOWEST_PRECEDENCE - 10)	// bluesky boot의 CoreMvcExceptionHandler(LOWEST_PRECEDENCE)보다 먼저 처리되어야 함
@ControllerAdvice
public class DmtExceptionHandler {
	...
	/**
	 * BrickException을 htmx fragment 요청에서 에러 메시지 fragment로 반환한다.
	 * fragment가 아닌 요청은 brick 기본 처리(에러 페이지 redirect)를 그대로 따른다.
	 */
	@ExceptionHandler
	public <T extends BlueskyException> Object handleException(T exception, HandlerMethod handlerMethod, NativeWebRequest nativeWebRequest) {
		...
		if (!isHtmxRequest(nativeWebRequest) || !ExceptionUtil.isHtmlResponse(handlerMethod, nativeWebRequest)) {
		...
		if (properties == null || !(properties.get(EXCEPTION_PARAMETER) instanceof BlueskyErrorMessage blueskyErrorMessage)) {
			return ExceptionUtil.handleException(problemDetail, handlerMethod, nativeWebRequest);
		}
	 ...
	 */
	@ExceptionHandler
	public <T extends UndeclaredThrowableException> Object handleException(T exception, HandlerMethod handlerMethod, NativeWebRequest nativeWebRequest) {
		return ExceptionUtil.handleException(ProblemDetailUtil.getProblemDetail(exception.getCause()), handlerMethod, nativeWebRequest);
	}
