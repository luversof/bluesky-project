...
public class SettingTest implements GeneralTest {
	...
	private static final Logger log = LoggerFactory.getLogger(SettingTest.class);
	...
	void fieldServiceFind() {
		var fieldPage = fieldService.findList(
				new SettingParameter(EventAdminConstant.ADMIN_PROJECT_ID_VALUE, EventAdminConstant.PROJECT_ID_VALUE,
						EventAdminConstant.MAINMENU_ID_VALUE, EventAdminConstant.SUBMENU_ID_VALUE_PROJECT));
	...
	void jdbcTemplateProjectSelectTest() {
		...
		argList.add("test");
		Project product = jdbcTemplate.queryForObject("select * from Project where projectId = ?",
				new ProjectRowMapper(), argList.toArray());
	...
	void namedParameterJdbcTemplateProjectSelectTest() {
		...
		var project = namedParameterJdbcTemplate.queryForObject("select * from Project where projectId = :projectId",
				paramSource, new ProjectRowMapper());
	...
	void namedParameterJdbcTemplateProjectSelectTest2() {
		...
		Project project = namedParameterJdbcTemplate.queryForObject(
				"select * from Project where projectId = :projectId", paramSource, new ProjectRowMapper());
		log.debug("product : {}", project);
	}
	...
	void namedParameterJdbcTemplateProjectSelectListTest() {
		...
		// List<Project> projectList = namedParameterJdbcTemplate.query("select * from
		// Project where projectId = :projectId", paramSource, new ProjectRowMapper());
		// List<Project> projectList = namedParameterJdbcTemplate.query("select * from
		// Project where projectId like '%:projectId%'", paramSource, new
		// ProjectRowMapper());
		// List<Project> projectList = namedParameterJdbcTemplate.query("select * from
		// Project where projectId like '%' + :projectId + '%'", paramSource, new
		// ProjectRowMapper());
		List<Project> projectList = namedParameterJdbcTemplate
				.query("select * from Project where projectId like :projectId", paramSource, new ProjectRowMapper());
	...
	void namedParameterJdbcTemplateProjectSelectCountTest() {
		var paramSource = new MapSqlParameterSource();
		paramSource.addValue("projectId", "noti");
		int result = namedParameterJdbcTemplate.queryForObject(
				"select COUNT(*) from Project where projectId = :projectId", paramSource, Integer.class);
