...
public class SettingTest implements GeneralTest {
  ...
  @Autowired private DbFieldServiceDecorator fieldService;
  ...
  void insertQuery() {
    ...
    String query =
        "UPDATE SubMenu SET queryString = :queryStriung, dataSourceName = :dataSourceName, dbType = :dbType where productId = :productId AND mainMenuId = :mainMenuId AND subMenuId = :subMenuId";
    ...
    var fieldPage =
        fieldService.findList(
            new SettingParameter(
                ...
                EventAdminConstant.SUBMENU_ID_VALUE_PROJECT));
  ...
  void jdbcTemplateProjectSelectTest() {
    ...
    Project product =
        jdbcTemplate.queryForObject(
            "select * from Project where projectId = ?", new ProjectRowMapper(), argList.toArray());
  ...
  /** 다른 object로 parameter를 만드는 방법 다만 여러 bean에서 가져와서 만들지는 못하는 듯? */
  @Test
  ...
  void namedParameterJdbcTemplateProjectSelectListTest() {
    ...
    List<Project> projectList =
        namedParameterJdbcTemplate.query(
            "select * from Project where projectId like :projectId",
            paramSource,
            new ProjectRowMapper());
  ...
  @Autowired private EventAdminMariadbProjectService eventAdminMariadbProductService;

  @Test
  void eventAdminMysqlProductServiceTest() {
    Project project =
        eventAdminMariadbProductService.findOne(new SettingParameter("admin", "noti", null, null));
