...
StockCharts.createChart = function (canvasId, config, existingInstance) {
  ...
  try {
    if (
      existingInstance &&
      Array.isArray(existingInstance.__copilotResizeTimers)
    ) {
      existingInstance.__copilotResizeTimers.forEach((timerId) =>
  ...
  try {
    const resizeTarget =
      canvas.closest(".amount-chart") || canvas.parentElement || canvas;
