(() => {
    if (window.__stockSelectionStorageAttached)
        return;
    ...
    function getAppConfigValue(name, fallback = "") {
        ...
        try {
            ...
            return ((_a = appConfig === null || appConfig === void 0 ? void 0 : appConfig.dataset) === null || _a === void 0 ? void 0 : _a[name]) || fallback;
        }
        catch (e) {
            return fallback;
        }
        ...
        try {
            ...
            if (acc) {
                if (acc.multiple) {
                    ...
                    const txt = (acc.options[acc.selectedIndex] &&
            ...
            if (stk) {
                if (stk.multiple) {
                    ...
                    const txt = (stk.options[stk.selectedIndex] &&
            ...
            if (tag) {
                if (tag.multiple) {
                    ...
                    const txt = (tag.options[tag.selectedIndex] &&
    ...
    function restoreToForm(form) {
        try {
            ...
            let restored = false;
            if (DEBUG)
                console.debug("[selectionStorage] restoreToForm start", form && form.id, obj);
            if (acc &&
                obj &&
                Array.isArray(obj.accountIdList) &&
                obj.accountIdList.length > 0) {
                if (acc.multiple) {
                    const current = new Set(Array.from(acc.selectedOptions).map((o) => o.value));
                ...
                if (DEBUG)
                    console.debug("[selectionStorage] restored account", form && form.id, restored);
            }
            if (stk &&
                obj &&
                ...
                if (stk.multiple) {
                    const current = new Set(Array.from(stk.selectedOptions).map((o) => o.value));
                ...
                if (DEBUG)
                    console.debug("[selectionStorage] restored stock", form && form.id, restored);
            }
            if (tag &&
                obj &&
                ...
                if (tag.multiple) {
                    const current = new Set(Array.from(tag.selectedOptions).map((o) => o.value));
                ...
                if (DEBUG)
                    console.debug("[selectionStorage] restored tag", form && form.id, restored);
            }
            ...
            try {
                if (restored &&
                    ...
                    try {
                        ...
                        else if (lastInjectionTimestamp &&
                    ...
                    if (!skipAuto) {
                        if (DEBUG)
                            console.debug("[selectionStorage] auto-submit scheduled", form && form.id);
                        setTimeout(() => {
                            try {
                        ...
                        if (DEBUG)
                            console.debug("[selectionStorage] skipped auto-submit (injection detected)", form && form.id);
                    }
                }
            }
            catch (e) {
            ...
            if (DEBUG)
                console.debug("[selectionStorage] restoreToForm end", form && form.id, restored);
        }
        catch (e) {
            /* ignore */
        }
    ...
    function restoreForAllForms() {
        try {
            ...
            forms.forEach((f) => {
                if (f.querySelector('select[name="accountIdList"]') ||
    ...
    function migratePerFormKeys() {
        try {
            ...
            if (accMap.size > 0 || stkMap.size > 0 || tagMap.size > 0) {
                ...
                for (const [id, text] of accMap)
                    out.accountIdList.push({ id, text });
                for (const [id, text] of stkMap)
                    out.stockItemIdList.push({ id, text });
                for (const [id, text] of tagMap)
                    out.stockTagList.push({ id, text });
                try {
                    sessionStorage.setItem(GLOBAL_KEY, JSON.stringify(out));
                }
                catch (e) {
    ...
    document.addEventListener("htmx:configRequest", function (e) {
        try {
            const ev = e;
            let params = ev.detail && ev.detail.parameters ? ev.detail.parameters : null;
            ...
            try {
                if (triggerElt instanceof Element) {
                    ...
                    const attrReset = triggerElt.getAttribute &&
                    ...
                    if (attrReset === "true" || t === "초기화") {
                        ...
                        try {
                            const formEl = triggerElt.closest("form") ||
                                document.querySelector("form");
                            if (formEl) {
                                const accSel = formEl.querySelector('select[name="accountIdList"]');
                                const stkSel = formEl.querySelector('select[name="stockItemIdList"]');
                                const tagSel = formEl.querySelector('select[name="stockTagList"]');
                                if (accSel) {
                                    if (accSel.multiple) {
                                        Array.from(accSel.options).forEach((o) => (o.selected = false));
                                    }
                                    else {
                                        try {
                                            accSel.value = "";
                                ...
                                if (stkSel) {
                                    if (stkSel.multiple) {
                                        Array.from(stkSel.options).forEach((o) => (o.selected = false));
                                    }
                                    else {
                                        try {
                                            stkSel.value = "";
                                    ...
                                    stkSel.dispatchEvent(new Event("change", { bubbles: true }));
                                }
                                if (tagSel) {
                                    if (tagSel.multiple) {
                        ...
                        if (DEBUG)
                            console.debug("[selectionStorage] reset trigger detected - cleared stored selection and removed params", { text: t, attrReset });
                        return;
                    }
                }
            }
            ...
            try {
                if (triggerElt instanceof Element) {
                    const attrSave = triggerElt.getAttribute &&
                        triggerElt.getAttribute("data-selection-save");
                    const searchLabel = getAppConfigValue("commonButtonSearch", "Search");
            ...
            try {
                ...
                try {
                    const path = ev.detail && ev.detail.path ? String(ev.detail.path) : null;
                    if (path) {
                            ...
                            if (path.indexOf(kw) !== -1) {
                                try {
                                    ...
                                    if (changed) {
                                        ...
                                        if (DEBUG)
                                            console.debug("[selectionStorage] sanitized path for excluded endpoint", { oldPath: path, newPath, kw });
                                    }
                                }
                                catch (e) {
                ...
                // skip injection.
                if (triggerElt instanceof Element) {
                    const hxAncestor = (triggerElt.closest &&
                    ...
                    if (hxAttr) {
                        for (const kw of excluded) {
                            if (hxAttr.indexOf(kw) !== -1) {
                                if (DEBUG)
                                    console.debug("[selectionStorage] skipping injection for excluded endpoint", { hxAttr, kw });
                                return;
                            }
                        }
                    }
            ...
            if (injected) {
                ...
                const form = triggerElt instanceof Element
                    ? triggerElt.closest("form")
                    : null;
                if (form)
                    injectedForms.add(form);
                lastInjectionTimestamp = Date.now();
                if (DEBUG)
                    console.debug("[selectionStorage] injected params into htmx request", { formId: form && form.id, params });
            }
        }
        catch (e) {
            /* ignore */
    ...
    document.addEventListener("htmx:beforeSwap", function (e) {
        try {
            ...
            try {
                ...
                if (wrapper.querySelector("html") ||
                    wrapper.querySelector("head") ||
                    wrapper.querySelector("body")) {
                    if (DEBUG)
                        console.debug("[selectionStorage] beforeSwap: server returned full document, aborting swap", target.id);
                ...
                if (!hasElements || (wrapper.textContent || "").trim().length === 0) {
                    if (DEBUG)
                        console.debug("[selectionStorage] beforeSwap: empty server response, aborting swap", target.id);
                    detail.shouldSwap = false;
                    return;
                }
                // Otherwise it's likely a fragment intended for innerHTML replacement — allow.
                if (DEBUG)
                    console.debug("[selectionStorage] beforeSwap: fragment response without target id — allowing inner swap", target.id);
            }
            catch (err) {
