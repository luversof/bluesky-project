...
(function () {
    ...
    function enhance(select) {
        if (!select || select.dataset.msd === "1")
            return;
        if (select.getAttribute("data-no-multi") === "1")
            return;
        ...
        var emptyArr = options.filter(function (o) {
            return !o.value;
        });
        ...
        select.__msdSync = syncFromSelect;
    ...
    function enhanceTag(card) {
        if (!card || card.dataset.msd === "1")
            return;
        card.dataset.msd = "1";
        var tagSelect = card.querySelector("[data-stock-tag-select]");
        // 태그 select 는 칩이 UI 라 항상 숨김(어떤 규칙이 보이게 해도 인라인 !important 로 강제).
        if (tagSelect)
            tagSelect.style.setProperty("display", "none", "important");
        var clearBtn = card.querySelector("[data-stock-tag-clear]");
        var labelEl = card.querySelector("[data-stock-tag-label]");
        var allLabel = (clearBtn && clearBtn.textContent ? clearBtn.textContent : "전체").trim();
        var tagLabelText = (labelEl && labelEl.textContent ? labelEl.textContent : "태그").trim();
        ...
        card.classList.remove("rounded-2xl", "border", "border-base-200", "bg-base-200/35", "px-3", "py-3");
        function updateSummary() {
            var selected = [];
            if (tagSelect) {
        ...
        if (tagSelect) {
            ...
            tagSelect.__msdSync = updateSummary;
        }
        toggle.addEventListener("click", function (e) {
            e.stopPropagation();
            closeAllPanels(panel);
    ...
    document.addEventListener("click", function (e) {
        ...
        setTimeout(function () {
            ...
            var stockSel = scope.querySelector('select[name="stockItemIdList"]');
            if (stockSel && typeof stockSel.__msdSync === "function")
                stockSel.__msdSync();
            var tagSel = scope.querySelector('select[name="stockTagList"]');
            if (tagSel && typeof tagSel.__msdSync === "function")
                tagSel.__msdSync();
        }, 0);
    });
    function enhanceAll() {
        NAMES.forEach(function (name) {
            var sels = document.querySelectorAll('select[multiple][name="' + name + '"]');
            Array.prototype.slice.call(sels).forEach(function (s) {
                enhance(s);
            });
        });
        Array.prototype.slice
