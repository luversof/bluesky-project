...
public class PoeTreeGraphService {
  ...
  private record PoeTreeFile(List<TreeNode> nodes, List<List<Integer>> edges) {}
  ...
  public synchronized void reload() {
    ...
    try (InputStream inputStream = Files.newInputStream(dataFile)) {
      ...
      logger.info("PoE 패시브 트리 그래프 로드: 노드 {}, 시작점 {}", nodes.size(), starts.keySet());
    } catch (Exception e) {
  ...
  public List<Integer> shortestPath(Set<Integer> allocated, int targetId) {
    ...
    while (!queue.isEmpty()) {
      ...
      for (int next : adjacency.getOrDefault(current, List.of())) {
        if (visited.contains(next) || !isTraversable(next, targetId)) {
          continue;
        }
        visited.add(next);
  ...
  private boolean isTraversable(int nodeId, int targetId) {
