...
(function () {
	...
	function modsMatching(alt: string, english: boolean): MapMod[] {
		...
		return mods.filter((m) => {
			const text = runtimeText(m, english);
			if (re) return re.test(text);
			return text.indexOf(alt) !== -1; // 정규식으로 못 읽으면 리터럴로
		});
	}
	...
	function renderList(): void {
		...
		for (const m of mods) {
			...
			if (m.quant > 0 && state.tab === "normal") {
				const qb = document.createElement("span");
				qb.className = "badge badge-ghost badge-xs font-mono";
				qb.textContent = "+" + m.quant + "%";
				qb.title = uiKo ? "아이템 수량(최대 롤)" : "Item quantity (max roll)";
