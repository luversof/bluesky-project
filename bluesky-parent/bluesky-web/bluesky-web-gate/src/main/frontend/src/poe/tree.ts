...
(() => {
	...
	function decodeTree(
		s: string,
	): { classId: number; ascend: number; bloodline: number; nodes: number[]; masteries: { node: number; effect: number }[] } | null {
		try {
	...
	function loadFromUrl() {
		...
		if (t) {
			...
			if (dec) {
				...
				for (const m of dec.masteries) masteryPicks.set(m.node, m.effect);
				return;
			}
		}
