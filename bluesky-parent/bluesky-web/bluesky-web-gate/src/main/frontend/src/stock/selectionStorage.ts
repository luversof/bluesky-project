...
interface Window {
	__stockSelectionStorageAttached?: boolean;
	stockSelectionStorage?: {
		saveFromForm: (form: HTMLFormElement | null) => void;
		restoreToForm: (form: HTMLFormElement | null) => void;
		GLOBAL_KEY: string;
	};
}

type SelectionEntry = { id: string; text?: string };
...
(() => {
	if ((window as any).__stockSelectionStorageAttached) return;
	(window as any).__stockSelectionStorageAttached = true;

	const GLOBAL_KEY = "__stockSelection:global";
	const DEBUG = (window as any).__stockSelectionStorageDebug === true;
	...
	function saveFromForm(form: HTMLFormElement | null): void {
		try {
			if (!form) return;
			const acc = form.querySelector<HTMLSelectElement>(
	...
	function restoreToForm(form: HTMLFormElement | null): void {
		try {
			if (!form) return;
			const raw = sessionStorage.getItem(GLOBAL_KEY);
			if (!raw) return;
			const obj = JSON.parse(raw) as SelectionObject;
			const acc = form.querySelector<HTMLSelectElement>(
				'select[name="accountIdList"]',
			);
			const stk = form.querySelector<HTMLSelectElement>(
	...
	function restoreForAllForms(): void {
		try {
	...
	function migratePerFormKeys(): void {
		try {
			...
			try {
				...
				if (curRaw) {
					const cur = JSON.parse(curRaw) as SelectionObject;
		...
		function (e: Event) {
			try {
				...
				try {
					if (triggerElt instanceof Element) {
						...
						if (attrReset === "true" || t === "초기화") {
							...
							try {
								const formEl =
									((triggerElt as Element).closest(
										"form",
									) as HTMLFormElement | null) ||
									document.querySelector("form");
								if (formEl) {
									const accSel = formEl.querySelector<HTMLSelectElement>(
										'select[name="accountIdList"]',
									);
					...
					if (triggerElt instanceof Element) {
						const hxAncestor = (triggerElt.closest &&
							(triggerElt.closest("[hx-get]") ||
								...
								triggerElt.closest("[hx-delete]"))) as Element | null;
				...
				if (injected) {
					...
					const form =
						triggerElt instanceof Element
							? (triggerElt.closest("form") as HTMLFormElement | null)
							: null;
	...
	document.addEventListener("htmx:beforeSwap", function (e: Event) {
		try {
			const ev = e as any;
			const detail = ev.detail;
			if (!detail) return;
			const target: Element | null = detail.target || null;
			const serverResponse: string | null = detail.serverResponse || null;
	...
	(window as any).stockSelectionStorage = {
		saveFromForm,
		restoreToForm,
		GLOBAL_KEY,
	};
