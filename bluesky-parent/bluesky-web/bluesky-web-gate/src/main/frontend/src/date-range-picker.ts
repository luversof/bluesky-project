    ...
    function create(cfg: any) {
        ...
        function updatePrevNextState() {
            try {
                ...
                const prevEls = Array.from((root as Element).querySelectorAll("." + prevClassName)) as HTMLButtonElement[];
                const nextEls = Array.from((root as Element).querySelectorAll("." + nextClassName)) as HTMLButtonElement[];
                const start = getStart();
                const end = getEnd();
                const mode = getMode();
                const maxDate = new Date(maxDateStr() + "T00:00:00");
                ...
                prevEls.forEach((el) => {
                    ...
                    if (disablePrev) {
                        ...
                        try { el.style.opacity = "0.2"; } catch (e) {}
                    } else {
        ...
        function getEnd(): string {
            ...
            if (instEl && instEl.value) {
                try {
                    const dt = new Date(instEl.value);
                    if (!isNaN(dt.getTime())) return fmtDate(new Date(dt.getFullYear(), dt.getMonth(), dt.getDate()));
                } catch (e) {}
            }
            return "";
        }
        ...
        function applyRange(startStr: string, endStr: string, modeStr: string) {
            ...
            try {
                if (isCallback()) {
                    ...
                    const se = el(cfg.startId) as HTMLInputElement | null; const ee = el(cfg.endId) as HTMLInputElement | null; const me = el(cfg.rangeModeId) as HTMLInputElement | null;
                    if (se) se.value = startStr || ""; if (ee) ee.value = endStr || ""; if (me) me.value = modeStr || "";
                    try {
                        function localDateToInstantIso(ds: string, addDays?: number) { if (!ds) return ''; const parts = ds.split('-'); const y = Number.parseInt(parts[0], 10); const m = Number.parseInt(parts[1], 10) - 1; const d = Number.parseInt(parts[2], 10); const dt = new Date(y, m, d + (addDays || 0), 0, 0, 0, 0); return dt.toISOString(); }
                        if (cfg.instantStartId) { const instSe = el(cfg.instantStartId) as HTMLInputElement | null; if (instSe) instSe.value = startStr ? localDateToInstantIso(startStr, 0) : ''; }
                    ...
                    if (form) {
                        if (typeof form.requestSubmit === 'function') form.requestSubmit();
                            ...
                            try { const ev = new Event('submit', { bubbles: true, cancelable: true }); const prevented = !form.dispatchEvent(ev); if (!prevented) { const tmp = document.createElement('button'); tmp.type = 'submit'; tmp.style.display = 'none'; form.appendChild(tmp); tmp.click(); tmp.remove(); } } catch (e) { try { const tmp2 = document.createElement('button'); tmp2.type = 'submit'; tmp2.style.display = 'none'; form.appendChild(tmp2); tmp2.click(); tmp2.remove(); } catch (e2) { form.submit(); } }
                        }
                    }
                }
            } catch (e) {}
        ...
        function applyRangeNoSubmit(startStr: string, endStr: string, modeStr: string) {
            try {
                const se = el(cfg.startId) as HTMLInputElement | null; const ee = el(cfg.endId) as HTMLInputElement | null; const me = el(cfg.rangeModeId) as HTMLInputElement | null;
                if (se) se.value = startStr || ''; if (ee) ee.value = endStr || ''; if (me) me.value = modeStr || '';
                try {
        ...
        function doSet(months: any, btn: Element | null) {
            clearActive();
            if (btn) { try { btn.classList.add(activeClass()); btn.classList.remove('btn-ghost'); } catch (e) {} }
            ...
            if (months === 0) { startStr = ''; endStr = ''; modeStr = 'all'; }
                ...
                if (atDataStart) {
                    const minD = new Date(cfg.minDate + 'T00:00:00'); let e = new Date(minD); e.setMonth(e.getMonth() + months); if (e > maxDate) e = new Date(maxDate); startStr = fmtDate(minD); endStr = fmtDate(e);
                } else { const s = new Date(maxDate); s.setMonth(s.getMonth() - months); startStr = fmtDate(s); endStr = maxStr; }
                modeStr = String(months);
            }
            try { const root = cfg.rootSelector ? document.querySelector(cfg.rootSelector) || document : document; btns(root).forEach((b) => { b.classList.remove(activeClass()); b.classList.add('btn-ghost'); }); if (btn) { btn.classList.add(activeClass()); btn.classList.remove('btn-ghost'); } } catch (e) {}
            applyRange(startStr, endStr, modeStr);
        }

        function doJumpToEdge(direction: string) {
            const mode = getMode(); if (mode === 'all') return; const maxStr = maxDateStr(); const maxDate = new Date(maxStr + 'T00:00:00'); clearActive(); let startStr = '', endStr = '';
            if (direction === 'end') {
                endStr = maxStr;
                if (mode && !isNaN(Number(mode)) && +mode > 0) { const s = new Date(maxDate); s.setMonth(s.getMonth() - +mode); startStr = fmtDate(s); }
                else if (mode === 'mtd') { startStr = fmtDate(new Date(maxDate.getFullYear(), maxDate.getMonth(), 1)); }
                else if (mode === 'ytd') { startStr = maxDate.getFullYear() + '-01-01'; }
                else { const cs = getStart(), ce = getEnd(); if (!cs || !ce) return; const ms = new Date(ce + 'T00:00:00').getTime() - new Date(cs + 'T00:00:00').getTime(); if (ms <= 0) return; startStr = fmtDate(new Date(maxDate.getTime() - ms)); }
            } else {
                if (!cfg.minDate) return; const minD = new Date(cfg.minDate + 'T00:00:00');
                if (mode && !isNaN(Number(mode)) && +mode > 0) { let e = new Date(minD); e.setMonth(e.getMonth() + +mode); if (e > maxDate) e = new Date(maxDate); startStr = fmtDate(minD); endStr = fmtDate(e); }
                else if (mode === 'mtd') { const first = new Date(minD.getFullYear(), minD.getMonth(), 1); const last = new Date(minD.getFullYear(), minD.getMonth() + 1, 0); startStr = fmtDate(first); endStr = fmtDate(last > maxDate ? maxDate : last); }
                else if (mode === 'ytd') { const minYear = minD.getFullYear(); startStr = minYear + '-01-01'; endStr = minYear === maxDate.getFullYear() ? maxStr : minYear + '-12-31'; }
                else { const cs = getStart(), ce = getEnd(); if (!cs || !ce) return; const ms = new Date(ce + 'T00:00:00').getTime() - new Date(cs + 'T00:00:00').getTime(); if (ms <= 0) return; let ne = new Date(minD.getTime() + ms); if (ne > maxDate) ne = new Date(maxDate); startStr = fmtDate(minD); endStr = fmtDate(ne); }
            }
                ...
                if (raw) {
                    try {
                        ...
                        if (obj) {
                            ...
                            try {
                                const candidates = Array.from((root as Element).querySelectorAll("." + (cfg.btnClass || "")));
                                for (const c of candidates) {
                                    const onclick = c.getAttribute('onclick') || '';
                                    if (obj.mode) {
                                        if (onclick.indexOf('set(' + obj.mode + ',') !== -1 || onclick.indexOf("set('" + obj.mode + "'") !== -1) { foundBtn = c as Element; break; }
                                    }
                                }
                                    ...
                                    for (const c of candidates) {
                                        try {
                                            ...
                                            const computeRange = (monthsParam: any) => {
                                                const maxStr = maxDateStr(); const maxDate = new Date(maxStr + 'T00:00:00');
                                                const today = new Date(); today.setHours(0,0,0,0);
                                                let startStr = '', endStr = '', modeStr = '';
                                                if (monthsParam === 0) { startStr = ''; endStr = ''; modeStr = 'all'; }
                                                else if (monthsParam === 'mtd') { startStr = fmtDate(new Date(maxDate.getFullYear(), maxDate.getMonth(), 1)); endStr = maxStr; modeStr = 'mtd'; }
                                                else if (monthsParam === 'ytd') { startStr = maxDate.getFullYear() + '-01-01'; endStr = maxStr; modeStr = 'ytd'; }
                                                else {
                                                    const curEnd = getEnd(); const curStart = getStart(); const todayStr = fmtDate(today);
                                                    const atDataEnd = !curEnd || curEnd >= todayStr;
                                                    const atDataStart = !atDataEnd && !!cfg.minDate && !!curStart && curStart <= cfg.minDate;
                                                    if (atDataStart) {
                                                        const minD = new Date(cfg.minDate + 'T00:00:00'); let e = new Date(minD); e.setMonth(e.getMonth() + monthsParam); if (e > maxDate) e = new Date(maxDate);
                                                        startStr = fmtDate(minD); endStr = fmtDate(e);
                                                    } else { const s = new Date(maxDate); s.setMonth(s.getMonth() - monthsParam); startStr = fmtDate(s); endStr = maxStr; }
                                                    modeStr = String(monthsParam);
                                                }
                                                return { start: startStr, end: endStr, mode: modeStr };
                                            };
                                            const r = computeRange(months);
                                            if (r.start === obj.start && r.end === obj.end) { foundBtn = c as Element; obj.mode = r.mode || obj.mode; break; }
                            ...
                            try {
                                ...
                                if (foundBtn) { foundBtn.classList.add(activeClass()); foundBtn.classList.remove('btn-ghost'); }
                            } catch (e) {}
                            ...
                            try {
                                ...
                                if (!sessionStorage.getItem(appliedKey)) {
                                    if (isCallback()) {
                                        try { _s.start = obj.start || ''; _s.end = obj.end || ''; _s.mode = obj.mode || ''; if (typeof cfg.onApply === 'function') { cfg.onApply(_s.start, _s.end, _s.mode); } } catch (e) {}
                                    } else {
                                        const formEl = el(cfg.formId) as HTMLFormElement | null;
                                        if (formEl) { try { if (typeof formEl.requestSubmit === 'function') formEl.requestSubmit(); else formEl.submit(); } catch (e) {} }
                                        else { try { applyRange(obj.start || '', obj.end || '', obj.mode || ''); } catch (e) {} }
                                    }
        ...
        return {
            ...
            setState: (start: string, end: string, mode: string) => {
                if (isCallback()) { _s.start = start; _s.end = end; if (mode !== undefined && mode !== null && mode !== '') { _s.mode = mode; } }
                ...
                if (mode !== undefined && mode !== null && mode !== '') {
                    try {
                        const root = cfg.rootSelector ? document.querySelector(cfg.rootSelector) || document : document;
                        btns(root).forEach((b: Element) => { try { b.classList.remove(activeClass()); b.classList.add('btn-ghost'); } catch (e) {} });
                        if (mode) {
                            try { Array.from((root as Element).querySelectorAll('.' + (cfg.btnClass || ''))).forEach((b: Element) => { const onclick = b.getAttribute && b.getAttribute('onclick') ? b.getAttribute('onclick') || '' : ''; if (onclick.indexOf('set(' + mode + ',') !== -1 || onclick.indexOf("set('" + mode + "'") !== -1) { try { b.classList.add(activeClass()); b.classList.remove('btn-ghost'); } catch (e) {} } }); } catch (e) {}
                        }
                    } catch (e) {}
                }
            },
            getState: () => ({ start: getStart(), end: getEnd(), mode: getMode() }),
            canShift: (dir: number) => { try { return canShift(dir); } catch (e) { return false; } },
        };
    }
