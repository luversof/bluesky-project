...
function buildDonutData(tradeData: any[] = [], metric = 'profit', groupBy = 'stock') {
...
function makeDonutTooltipHandler(rawData: any[], isProfitMode: boolean, tooltipId = 'tradeDonutTooltipEl') {
...
StockCharts.initMonthlyFromData = function(tradeData: any[], canvasId = 'tradeMonthlyChart', existingInstance?: any) {
  const m = buildMonthlyData(tradeData);
  const ctx = document.getElementById(canvasId) as HTMLCanvasElement | null;
  if (!ctx) return null;
  if (existingInstance) try { existingInstance.destroy(); } catch(e){}
  ...
  const inst = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: { callbacks: { label: (ctx: any) => { return ctx.dataset.label + ': \u20a9' + fmtAmt(ctx.parsed.y); } } }
      },
      scales: {
...
StockCharts.initDonutFromData = function(tradeData: any[], opts: any = {}, existingInstance?: any) {
  const metric = opts.metric || 'profit';
  const groupBy = opts.groupBy || 'stock';
  const canvas = document.getElementById(opts.canvasId || 'tradeDonutChart') as HTMLCanvasElement | null;
  ...
  if (d.labels.length === 0) {
    ...
    if (legendEl) legendEl.innerHTML = '<div class="text-xs opacity-40 pt-4 text-center">해당 기간 ' + (isProfitMode ? '매도 내역' : '매수 내역') + '이 없습니다.</div>';
    return null;
  }
  ...
  if (legendEl) {
    ...
    legendEl.innerHTML = d.labels.map((l: string, i: number) => {
...
StockCharts.createChart = function(canvasId: string, config: any, existingInstance?: any) {
