 ...
 * 부풀고 수익률은 그만큼 낮게 보인다.
 *
 * <p><b>실제로 그 사고가 있었다</b> &mdash; 2026-08-22 실측: 이 화면만 세금을 빠뜨려 두 계좌(한국투자증권 위탁 · ISA)의 매도 원가가 그 계좌
 * 증권거래세 합만큼 과대 계상됐다.
 *
...
class RealizedRateDenominatorRenderTest {
  ...
  void 종목_실현손익률의_분모가_매도금액에서_세금과_실현손익을_뺀_값이다() {
    ...
    assertThat(html)
        .as("매도 원가에서 증권거래세를 빠뜨리면 수익률이 낮게 보인다 (실측 사고: 거래세만큼 원가 과대)")
        .contains("+10.78%")
   ...
   * 2026-08-24: 계좌별과 종목별이 6.79% 어긋났다. 매도금액·실현손익·거래세는 정확히 일치한다).
   *
   * <p>같은 종목을 한 계좌에서만 팔고 다른 계좌에선 들고 있으면 어떤 필터로도 두 값을 맞출 수 없다 &mdash; 계좌 쪽으로 좁히면 이번엔 반대 방향으로 벌어진다.
   * 그래서 맞추는 대신, 옆 칸 수익률의 실제 분모인 매도원가로 바꿨다.
   */
  @Test
