 ...
 * ofStockRealized} 가 두 값을 한 자리에 담는 바람에 같은 화면 안에서 헤드라인·거래목록(기록값 합계)과 계좌별·종목별 표가 0.11% 어긋났다(실측). 두 값을
 * 분리해 싣고, 표시는 기록값으로 통일한다.
 */
class TradeProfitStockRealizedTest {
  ...
  private TradeProfit stockRealized(String recorded, String computedNet) {
    return TradeProfit.ofStockRealized(
        ...
        new BigDecimal("400050000"),
        new BigDecimal("199500000"),
  ...
  void 기록된_실현손익과_앱이_계산한_실현손익을_각각_보존한다() {
    TradeProfit row = stockRealized("120000000", "119990000");

    assertThat(row.realizedProfit()).isEqualByComparingTo("120000000");
    assertThat(row.realizedProfitNet()).isEqualByComparingTo("119990000");
  ...
  void 집계는_두_기준을_섞지_않는다() {
    var sums =
        TradeProfitAggregator.aggregate(
            List.of(stockRealized("120000000", "119990000"), stockRealized("570", "9438")));

    assertThat(sums.realizedProfit()).isEqualByComparingTo("120000570");
    assertThat(sums.realizedProfitNet()).isEqualByComparingTo("119999438");
  }
  ...
  void 보유_수량과_매도_수량은_그대로_실린다() {
    ...
    assertThat(row.totalSellAmount()).isEqualByComparingTo("200000000");
    assertThat(row.totalSellProceeds()).isEqualByComparingTo("199500000");
  }
}
